...
class HybridVDBRetriever:
    ...
    def __init__(
        ...
        collection_type: str = "legal_cases",  # "legal_cases" or "legislation"
        ...
        Args:
            ...
            collection_type: Type of collection ("legal_cases" or "legislation")
            chroma_path: Path to ChromaDB storage (overrides config if provided)
            collection_name: Name of the collection (overrides config if provided)
        ...
        self.chroma_path = chroma_path or db_config.chroma.persist_directory
        self.collection_name = collection_name or db_config.chroma.collections.get(collection_type, "fyp1")
    ...
    def _initialize_vector_store(self):
        ...
        try:
            # Get ChromaDB client settings from config
            client_settings = db_config.chroma.client_settings
            
            if "persist_directory" in client_settings:
                # Local persistent storage
                persist_dir = self.chroma_path or client_settings["persist_directory"]
                ...
                logger.info(f"Using remote ChromaDB at: {client_settings['host']}:{client_settings['port']}")
    ...
    def switch_collection(self, collection_type: str, collection_name: Optional[str] = None):
        """
        Switch to a different collection without reinitializing the models.
        ...
        try:
            ...
            # Create new vector store with existing embedding model
            self.vector_store = ChromaVectorStore(
                chroma_collection=self.collection,
                embed_model=self.embed_model
            )
            
            # Reset retrievers (will be recreated when documents are ingested)
            self.vector_retriever = None
            self.bm25_retriever = None
            self.hybrid_retriever = None
            self.query_engine = None
...
def main():
    """Example usage of the HybridVDBRetriever with configuration."""
    # Initialize retriever for legal cases using config
    legal_cases_retriever = HybridVDBRetriever(
        collection_type="legal_cases"
    )
    
    # Initialize retriever for legislation using config
    legislation_retriever = HybridVDBRetriever(
        collection_type="legislation"
    )
    
    # You can still override config values if needed
    ...
    # Ingest documents
    legal_cases_retriever.ingest_documents(documents)
    
    # Query the system
    response = legal_cases_retriever.query("Is there anything related to car accidents?")
